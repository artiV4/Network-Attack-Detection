...
class Feature_extraction():
    ...
    def pcap_evaluation(self,pcap_file,csv_file_name):
        ...
        for ts, buf in (pcap):
            ...
            if eth.type == dpkt.ethernet.ETH_TYPE_IP or eth.type == dpkt.ethernet.ETH_TYPE_ARP:
                ...
                if eth.type == dpkt.ethernet.ETH_TYPE_IP:     # IP packets
                    ...
                    # print(f"Proto_type: {proto_type}; Class {proto_type.__class__}")
        ...
        # for element in list(processed_df):
        #     print(element)
        #     print(processed_df[element])
        # summary
        last_row = 0
        n_rows = 10
        df_summary_list = []
        while last_row<len(processed_df):
            ...
            numerical_cols = sliced_df.select_dtypes(include=["number"]) #1
            mean_values = numerical_cols.mean() #1
            sliced_df_mean = pd.DataFrame(mean_values).T #1
            for col in sliced_df.columns: #1
                if col not in numerical_cols.columns: #1
                    sliced_df_mean[col] = sliced_df[col].iloc[0] #1

            # sliced_df = pd.DataFrame(sliced_df.mean()).T# mean #1
